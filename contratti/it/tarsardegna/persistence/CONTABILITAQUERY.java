...
public enum CONTABILITAQUERY implements PersistenceQuery {
	...
	MAIN_QUERY(
			"SELECT f.id, cap.denominazione AS capitolo, f.numero, "
					...
					+ "WHERE NOT f.annullato;", new FatturaMapper()), GENERAL_FATTURA_UPDATE_QUERY(
			"UPDATE fattura SET " + DB.updatePattern + " = ? WHERE id = ?",
			null), FATTURA_CAPITOLO_UPDATE_QUERY(
			"UPDATE fattura SET capitoloid = ?, vocespesaid=NULL WHERE id = ?",
			null), FATTURA_DATADECRETO_UPDATE_QUERY(
			"UPDATE decreto SET datadecreto = ?  WHERE id IN (SELECT f.decretoid FROM fattura f WHERE f.id = ?)",
			null), FATTURA_NULL_VOCESPESA_UPDATE_QUERY("UPDATE fattura "
			+ "SET vocespesaid = s.voceid " + "FROM ("
			...
			+ "datasdi, scadenza, eserciziospesa, vocespesaid, autoreid FROM fattura WHERE id = ? RETURNING id; ",
			null);
